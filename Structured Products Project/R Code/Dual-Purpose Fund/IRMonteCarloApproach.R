IRMonteCarloApproach = function(s,t,tr,V01,maxt,nt,sub_nt,nm,hu,hd,R,C,r,c,sig,alpha)
{
  #Notes:
  ...
  for (i in 1:nm){
    ...
    for (j in 1:(tnt-1)){
      if (j%%365==0) {
        if (j==365) {spath[j+1]=spath[j]*exp((r-c-sig^2/2)*deltat+sig*sqrt(deltat)*ram[j]) - alpha*R*(1-tr)}
          else {spath[j+1]=spath[j]*exp((r-c-sig^2/2)*deltat+sig*sqrt(deltat)*ram[j]) - alpha*R}
        }
    ...
    for (j in 1:(tnt-1)){ # check if upside or downside is touched
      if (j*deltat+t<=1) {e=1} else {e=0}
      if((spath[j]>=hu)||(spath[j]<=(alpha*(1+R*(j*deltat+t-floor(j*deltat+t)))+(1-alpha)*hd - alpha*tr*R*e))){q=1; break}
    }
    if (q==1){ # touch upside or downside
      j=1
      repeat {t1=j;if ((spath[j]>=hu)||(j>=tnt)) break;j=j+1}
      j=1
      repeat {t2=j;if (j*deltat+t<=1) {e=1} else {e=0}; if ((spath[j]<=(alpha*(1+R*(j*deltat+t-floor(j*deltat+t)))+(1-alpha)*hd - alpha*tr*R*e))||(j>=tnt)) break;j=j+1}
      
      
      if (floor(t1*deltat+t)==0){accR1 = 0} else {accR1 = -exp(-r*(1-t))*tr*R; for(j in 1:floor(t1*deltat+t)){accR1 = accR1 + exp(-r*(j-t))*R}}
      if (floor(t2*deltat+t)==0){accR2 = 0} else {accR2 = -exp(-r*(1-t))*tr*R; for(j in 1:floor(t2*deltat+t)){accR2 = accR2 + exp(-r*(j-t))*R}}
      if (t1<t2){ # upside is touched first
        if (floor(t1*deltat+t)==0){price[i] = accR1 + exp(-r*t1*deltat)*(R*(t1*deltat+t-floor(t1*deltat+t)-tr)+V01)}
        else{price[i] = accR1 + exp(-r*t1*deltat)*(R*(t1*deltat+t-floor(t1*deltat+t))+V01)}
      }else { # downside is touched first
        if (floor(t2*deltat+t)==0){price[i] = accR2 + exp(-r*t2*deltat)*(R*(t2*deltat+t-floor(t2*deltat+t)-tr)+1-hd+hd*V01)}
        else{price[i] = accR2 + exp(-r*t2*deltat)*(R*(t2*deltat+t-floor(t2*deltat+t))+1-hd+hd*V01)}
      }
    }
  }
