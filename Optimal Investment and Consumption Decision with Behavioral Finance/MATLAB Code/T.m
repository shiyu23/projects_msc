function f=T(hx, hxrf, ht, N, mu, r, sig, xmax, xrfmin, xrfmax, lambda, alphag, alphal, pimin, pimax, cmax, thred, k)
...
if -lambda*abs(-hxrf*(j-1)*k) == max([-lambda*abs(-hxrf*(j-1)*k),((1+lambda)*ind(cmax,hxrf*(j-1)*k)-lambda)*abs(cmax-hxrf*(j-1)*k)-cmax*((phi(i+1,j)-phi(i-1,j))/2/hx+(phi(i,j+1)-phi(i,j-1))/2/hxrf),-hxrf*(j-1)*k*((phi(i+1,j)-phi(i-1,j))/2/hx+(phi(i,j+1)-phi(i,j-1))/2/hxrf)])*ind1(hxrf*(j-1)*k,0,cmax) + max([-lambda*abs(-hxrf*(j-1)*k),((1+lambda)*ind(cmax,hxrf*(j-1)*k)-lambda)*abs(cmax-hxrf*(j-1)*k)-cmax*((phi(i+1,j)-phi(i-1,j))/2/hx+(phi(i,j+1)-phi(i,j-1))/2/hxrf)])*(1-ind1(hxrf*(j-1)*k,0,cmax))
    c(i,j)=0;
elseif ((1+lambda)*ind(cmax,hxrf*(j-1)*k)-lambda)*abs(cmax-hxrf*(j-1)*k)-cmax*((phi(i,j)-phi(i-1,j))/hx+(phi(i,j)-phi(i,j-1))/hxrf) == max([-lambda*abs(-hxrf*(j-1)*k),((1+lambda)*ind(cmax,hxrf*(j-1)*k)-lambda)*abs(cmax-hxrf*(j-1)*k)-cmax*((phi(i+1,j)-phi(i-1,j))/2/hx+(phi(i,j+1)-phi(i,j-1))/2/hxrf),-hxrf*(j-1)*k*((phi(i+1,j)-phi(i-1,j))/2/hx+(phi(i,j+1)-phi(i,j-1))/2/hxrf)])*ind1(hxrf*(j-1)*k,0,cmax) + max([-lambda*abs(-hxrf*(j-1)*k),((1+lambda)*ind(cmax,hxrf*(j-1)*k)-lambda)*abs(cmax-hxrf*(j-1)*k)-cmax*((phi(i+1,j)-phi(i-1,j))/2/hx+(phi(i,j+1)-phi(i,j-1))/2/hxrf)])*(1-ind1(hxrf*(j-1)*k,0,cmax))
    c(i,j)=cmax;
else
    c(i,j)=k*hxrf*(j-1);
end
